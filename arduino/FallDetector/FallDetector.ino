//============================================================================
// Name		: FallDetector.ino
// Author(s)	: Barbara Bruno
// Affiliation	: University of Genova, Italy - dept. DIBRIS
// Version	: 1.0
// Description	: Embedded real-time fall detector
//============================================================================
...
const float PDT = 0.4*STD_GRAVITY;    // Posture Detection Threshold
const int BPT = 35;                   // Body Posture Threshold (degrees)
...
float bodyPosture;            // user body posture detected after a fall
//----------------------------------------------------------------------------
...
void loop()
{
  ...
  // 4. wait until the end of the fall to detect the final body posture
  if(realFall == true && abs(Amax - Amin) < PDT)
  {
    // detect the post-fall posture
    bodyPosture = acos(real_acceleration[0]/STD_GRAVITY) * 180.0 / PI;
    /*****************************************************
    // DEBUG: display the body posture angle
    Serial1.print(bodyPosture);
    *****************************************************/
    // signal the posture and lack/presence of motion
    // case 1: person lying AND not moving
    if(bodyPosture > BPT && abs(Amax - Amin) < MDT)
    {
      // transmit the fall alarm via ZigBee
      Serial1.print("F Lying still\n");
      Serial.print("F Lying still\n");
    }
    // case 2: person lying AND moving
    else if(bodyPosture > BPT && abs(Amax - Amin) >= MDT)
    {
      // transmit the fall alarm via ZigBee
      Serial1.print("F Lying moving\n");
      Serial.print("F Lying moving\n");
    }
    // case 3: person standing AND not moving
    else if(bodyPosture <= BPT && abs(Amax - Amin) < MDT)
    {
      // transmit the fall alarm via ZigBee
      Serial1.print("F Standing still\n");
      Serial.print("F Standing still\n");
    }
    // case 4: person standing AND moving
    else
    {
      // transmit the fall alarm via ZigBee
      Serial1.print("F Standing moving\n");
      Serial.print("F Standing moving\n");
    }
