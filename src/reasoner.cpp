...
//! @return:		---
void Reasoner::buildSet(string dF)
{
	string one_HMPn;
	float temp1;
	float temp2;
...
//! @return:	---
void Reasoner::publishReasoner(int& i)
{
	// HMP.[HMPname]
...
//! @return:	---
void Reasoner::reportReasoner(int& i, string rF)
{
	// create result file
	ofstream outputFile;
...
//! @param rF:			name of the result file
//! @return:			---
void Reasoner::updateInterval(int& i,int& nS,float &p,float &pp,int f,string rF)
{
	// 1) active
	if(p > 0)
	{
...
//! @return:			---
void Reasoner::offlineReasoner(string path, string testFile)
{
...
//! @return:		---
void Reasoner::onlineReasoner(char* port) //Online reasoner using tuple view
{
	int nSamples = 0;			// number of acquired samples
	...
	while(1) //Infinite loop
	{
		try
		{
			...
			if (nSamples >= hC.window_size)
			{
				...
				hC.publishDynamic(poss); //Displays in Tuple view
