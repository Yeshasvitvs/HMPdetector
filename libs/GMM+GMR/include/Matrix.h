...
public:
  ...
  inline Matrix(arma::mat m)
    {

      //row    = m.n_rows;
      //column = m.n_cols-1;
      _      = NULL;
      Resize(m.n_rows,m.n_cols,false);
      //DEBUG:std::cout<<"CREO "<<row<<" "<<column<<std::endl<<std::flush;
      for (unsigned int j = 0; j < row; j++)
        for (unsigned int i = 0; i < column; i++)
        {
        	 //std::cout<<m(j,i)<<std::endl<<std::flush;
        	 _[j*column+i] = m(j,i);
        }
    }
...
public:  
  inline virtual void Resize(unsigned int rowSize, unsigned int colSize, bool copy = true){
    if((row!=rowSize)||(column!=colSize)){
    	//std::cout<<"resizo"<<rowSize<<colSize<<std::endl;
      if((rowSize)&&(colSize)){
